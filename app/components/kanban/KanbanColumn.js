...
const KanbanColumn = {
  ...
  computed: {
    ...
    totalSystemSizeKw() {
      ...
      const total = this.projects.reduce((sum, project) => {
          // Access the raw kW_STC field and parse it
  ...
  methods: {
    ...
    handleDrop(event) {
      ...
      // Emit event with the column's ID (stage ID or tranche ID)
      this.$emit('column-drop'); // Board already knows the target column from the event binding
    },
    ...
    onCardDragStart(cardId) {
        ...
        // Emit project ID (already correct as cardId is project.ID) and column ID
        this.$emit('card-drag-start', cardId, this.stage.id); 
    },
    onCardDragEnd() {
    ...
    onCardClick(projectId) {
        // projectId is already the project.ID
        ...
        console.log(`KanbanColumn: Card clicked - Project ID: ${projectId}`);
    },
    // Method to emit toggle event
    toggleCollapse() {
        // Emit the column's ID (stage.id contains stage OR tranche ID)
  ...
  template: `
      ...
      >
        ...
        >
           ...
           <div :class="['flex items-center flex-shrink-0', isCollapsed ? 'flex-col-reverse space-y-1 space-y-reverse' : 'ml-1']">
                ...
                <div v-if="isCollapsed" class="flex flex-col items-center">
                    <base-badge color="gray">{{ projectCount }}</base-badge>
                    <!-- Hide size badge when collapsed -->
        ...
        <div 
            v-if="!isCollapsed"
            class="flex-grow overflow-y-auto p-2 space-y-2 min-h-[100px] scrollbar-hide"
            >
          <kanban-card
            ...
            v-on:dragstart="onCardDragStart(project.ID)"
